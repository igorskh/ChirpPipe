...
class MobileNetMerged(nn.Module):
    def __init__(self, block_size=4, authentic_weights_path=None, synthetic_weights_path=None):
        ...
        self.authentic = timm.create_model(
            "mobilenetv3_large_100.ra_in1k", pretrained=False)
        self.syntetic = timm.create_model(
            "mobilenetv3_large_100.ra_in1k", pretrained=False)

        if authentic_weights_path:
            self.authentic.load_state_dict(torch.load(
                authentic_weights_path, map_location=torch.device('cpu')))

        if synthetic_weights_path:
            self.syntetic.load_state_dict(torch.load(
                synthetic_weights_path, map_location=torch.device('cpu')))
